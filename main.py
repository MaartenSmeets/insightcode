...
def clean_diagram_code(diagram_code: str, diagram_type: str) -> str:
    """
    Clean the diagram code by removing code block markers and any additional text
    specific to the given diagram type.
    
    Parameters:
    - diagram_code (str): The raw diagram code generated by the LLM.
    - diagram_type (str): The type of diagram (e.g., 'mermaid', 'plantuml').
    
    Returns:
    - str: Cleaned diagram code without any unnecessary markers.
    """
    # Build a dynamic regex pattern to match any code block markers for the diagram type
    diagram_type = diagram_type.lower()
    block_marker_pattern = rf'```(?:{diagram_type})?\s*\n|```|:::(?:{diagram_type})?\s*\n|:::'

    # Remove code block markers like ```mermaid, ```plantuml, :::mermaid, :::plantuml
...
def fix_diagram_code_with_llm(diagram_code: str, error_message: str, diagram_type: str) -> str:
...
"""
    ...
    fixed_diagram_code = clean_diagram_code(fixed_diagram_code, diagram_type)
    ...
    try:
        ...
        if codebase_summary:
            ...
            if GENERATE_DIAGRAM:
                ...
                diagram_code = clean_diagram_code(diagram_code, OUTPUT_FORMAT)
                    ...
                    try:
                        ...
                        if attempt < MAX_FIX_ATTEMPTS:
                            ...
                            fixed_diagram_code = fix_diagram_code_with_llm(
                                diagram_codes[-1], error_message, OUTPUT_FORMAT
                            )
